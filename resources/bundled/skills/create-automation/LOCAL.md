...
# Optional (default true): active for future scheduling. Run now works
# either way (disabled just shows a warning).
enabled = true

# Required: cron expression or preset like "@daily". Stored only; not fired yet.
schedule = "0 9 * * 1-5"
...
# Optional, stored for future releases:
...
4. Confirm to the user: path, name, schedule string, runner - and that scheduling isn't live yet, so runs happen via Settings → Automations → **Run now** (offer that as an immediate smoke test).
