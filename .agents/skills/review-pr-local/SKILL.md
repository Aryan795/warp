...
- Audit every comment a PR adds or changes against the "Comments" guidance under "Development Guidelines" in `AGENTS.md`. Flag comments that narrate WHAT/HOW instead of a non-obvious WHY, line-by-line narrations, docstrings that describe internal steps or enumerate callers, explanations duplicated at a call site that the declaration's doc comment already covers, "transformation" comments describing the edit itself (e.g. "this used to...") rather than the current state, and existing comments removed as collateral of an unrelated change. Treat a clear violation as `⚠️ [IMPORTANT]`, not a nit. See `AGENTS.md` for the authoritative list rather than restating it here.
