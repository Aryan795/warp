...
Preserve every viewer prompt that the local network definitively cannot accept by moving it into
the existing conversation-keyed `QueuedQueryModel`. A transient reconnect retries the head prompt
into the same live shared session. A fatal ambient-session disconnect converts the head prompt into
a cloud follow-up when the viewer owns an eligible task; the remaining rows continue in FIFO order
after the replacement execution session has joined. The existing queued-prompts panel remains the
single durable and editable user surface; no second pending-prompt queue is introduced.

### User-visible behavior

1. Submitting a prompt while the viewer websocket is joined behaves as it does today. The prompt
   travels to the sharer, the viewer input remains frozen only until the existing in-flight
   acknowledgement, and no queued row is created.
2. If the viewer network rejects the prompt because the network is reconnecting, has closed, or
   cannot enqueue the server message, the exact prompt and its pending attachments appear once in
   the existing queued-prompts panel for that conversation.
...
15. Diagnostic logging reports every server message rejected locally by
    ...
    message kind/discriminant. It never logs prompt text, attachment names or contents, input
    buffer contents, or other user content.
...
- Persisting queued prompts across application restarts beyond `QueuedQueryModel`'s current
  lifecycle.
- A server protocol change, delivery receipt, or durable server-side prompt inbox. The selected
  contract covers messages the local client definitively rejects before accepting them into its
  websocket send path; the existing server `AgentPromptRequestInFlight` acknowledgement remains
  the post-acceptance UI signal.
...
#### 1. Make local network acceptance observable and content-safe

- Add a small result type for local network submission, for example
  `ServerMessageSendOutcome::{Accepted, Undeliverable}` or an equivalent `Result`.
- Make `Network::send_message_to_server` return that outcome:
  - return `Undeliverable` when the stage is not `JoinedSuccessfully`;
  - return `Undeliverable` when `messages_to_server_tx.try_send` fails;
  - return `Accepted` only after `try_send` accepts the message.
...
- `Accepted` means ownership transferred to the existing websocket proxy; it is not a new
  server-side delivery receipt. Keep `AgentPromptRequestInFlight` as the signal that clears the
  normal frozen viewer input.
...
- Add optional retry-target metadata to a prompt row rather than putting a second queue on
  `Network`:
  - original `SessionId`;
...
- On `Accepted`, do not append a fallback row. On `Undeliverable`, append or restore exactly one
  row and immediately exit the viewer loading state. Clear only the failed submission's editor
  presentation; never overwrite a newer draft.
...
- Complete the claim only on:
  - `Accepted` from the same-session viewer network; or
  - successful validation and start of `submit_cloud_followup`.
- On upload failure, network `Undeliverable`, stale target, ineligible cloud follow-up, or a
  disabled required feature, restore the exact query at its original relative position. If user
  edits/reorders/deletes while no claim is active, subsequent dispatch uses the current row.
...
- Deleting the queue row cancels the pending retry. Reordering changes later FIFO order. Editing
  changes the exact text/attachments used by the next attempt.

### Ordering and no-duplication invariants

1. `QueuedQueryModel` holds the only durable copy while a prompt is pending.
2. A prompt is either in the queue, exclusively claimed by one dispatch attempt, or accepted by
   one submission path. It is never in two of these states simultaneously.
...
4. `Accepted` by the old session and cloud-follow-up dispatch are mutually exclusive outcomes for
   one claim. Fatal teardown cannot see an already accepted/removed row; an undeliverable old send
   restores before the fatal drain can claim it.
...
- **Double-submit from racing events:** atomically take the head before async work, pass a typed
  retry token end to end, and restore only that token.
...
1. **Pre-fix bug reproduction:** Add a deterministic test that places a viewer `Network` in
   ...
   demonstrates the current behavior: the network does not accept the message, no queue row exists,
   and the input remains frozen. The test must fail before the implementation and pass afterward by
   asserting one unlocked queued row and an editable input.
2. **Network outcome contract:** In `viewer/network_tests.rs`, assert that
   `send_message_to_server`/`send_agent_prompt_request` returns `Undeliverable` for every
   non-joined stage and a closed/full send channel, and `Accepted` only when the local channel
   accepts the message.
...
6. **Happy-path no queue:** Submit the same plain and attachment prompts while joined. Assert the
   local channel receives one `AgentPromptRequest`, no fallback row is appended, and the existing
   in-flight acknowledgement unfreezes/clears the input as before.
7. **Transient rejoin:** In `terminal_manager_tests.rs`, queue at least two rows for the same
...
8. **Transient mismatch matrix:** Parameterize rejoin tests over old-network event, different
...
9. **Fatal first-row handoff:** Simulate reconnect exhaustion for an owned, resumable ambient task
...
10. **Ineligible fatal matrix:** Parameterize non-owner/read-only, blocked source,
...
11. **Replacement execution join:** Emit `ExecutionSessionReady` and assert no queued prompt is
...
12. **Ordering across all triggers:** Create three rows, race reconnect/fatal-end/join and an
    ...
    between two rows.
13. **Queued cloud attachments:** In `queued_prompts_tests.rs`/`input_tests.rs`, fire a queued cloud
...
14. **Attachment failure restoration:** Fail image upload, file upload, and task upload completion
...
15. **Queue controls and lifecycle:** Assert disconnected rows can be edited, deleted, reordered,
...
16. **Feature-off behavior:** Override queued-prompts, handoff, and image-context flags separately.
    ...
    proceed.
17. **Existing regressions:** Run focused tests for
...
18. **Repository gates:** From the repository root run the documented L/cross-cutting checks:
    - `./script/format --check`
...
19. **Running UI verification:** After a successful build, exercise the real Warp GUI with computer
