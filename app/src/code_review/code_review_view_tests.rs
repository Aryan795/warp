...
/// A file whose deleted-status is unchanged and has no unsaved edits keeps
/// its existing editor across `invalidate_all`, instead of the editor and
/// its buffer being torn down and rebuilt; a file that drops out of the new
/// diff is removed from the loaded state.
#[test]
fn invalidate_all_reuses_unchanged_file_editor_and_drops_removed_file() {
    App::test((), |mut app| async move {
        initialize_test_app(&mut app);
        app.add_singleton_model(warp_files::FileModel::new);
        app.add_singleton_model(GlobalBufferModel::new);

        let editor_unchanged = create_editor_with_content(&mut app, "line 1\nline 2\nline 3");
        let editor_to_remove = create_editor_with_content(&mut app, "goodbye");
        let unchanged_editor_id = editor_unchanged.id();

        let repo_path = PathBuf::from("/repo");
        let repo_key = LocalOrRemotePath::Local(repo_path.clone());

        let (window_id, _) = app.add_window(WindowStyle::NotStealFocus, |_| TestView);
        let initial_state = create_loaded_state_with_editors(
            &mut app,
            window_id,
            vec![
                ("unchanged.txt".to_string(), editor_unchanged),
                ("removed.txt".to_string(), editor_to_remove),
            ],
        );
        ...
        code_review_view.update(&mut app, |view, ctx| {
            if let Some(repo) = view.active_repo.as_mut() {
                repo.state = CodeReviewViewState::Loaded(initial_state);
            }

            // Force the underlying diff-state model into `Loaded` so
            // `invalidate_all` takes its normal reuse-eligible path instead
            // of bailing out early on `NotInRepository`.
            let diff_state_model = view.diff_state_model().clone();
            diff_state_model.update(ctx, |wrapper, model_ctx| {
                if let DiffStateModel::Local(local) = wrapper {
                    local.update(model_ctx, |model, _| model.set_loaded_for_test());
                }
            });
        });

        // Recompute the diff: "unchanged.txt" keeps the same Modified status
        // and content; "removed.txt" is no longer part of the diff at all.
        let diff_data = GitDiffWithBaseContent {
        ...
        code_review_view.update(&mut app, |view, view_ctx| {
            view.invalidate_all(Some(&diff_data), None, view_ctx);

            let CodeReviewViewState::Loaded(loaded) = view.state() else {
                panic!("expected Loaded state after invalidate_all");
            };

            assert_eq!(
                loaded.file_states.len(),
                1,
                "removed.txt should no longer be present after invalidate_all"
            );

            let reused_state = loaded
                .file_states
                .get("unchanged.txt")
                .expect("unchanged.txt should still be present");
            let reused_editor_id = reused_state
                .editor_state
                .as_ref()
                .expect("unchanged.txt should still have an editor")
                .editor()
                .id();

            assert_eq!(
                reused_editor_id, unchanged_editor_id,
                "unchanged.txt's editor should be REUSED (same ViewHandle id), not recreated"
            );

            assert!(
                !loaded.file_states.contains_key("removed.txt"),
                "removed.txt should be dropped from the loaded state"
            );
        });
    });
}
