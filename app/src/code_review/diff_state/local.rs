...
use warp_util::git::{BoundedGitOutput, run_git_command, run_git_command_bounded};
...
use crate::code_review::diff_size_limits::{
    DiffSize, MAX_DIFF_SIZE, MAX_STATUS_ENTRIES, METADATA_STATUS_BYTE_BUDGET, UnrenderableReason,
    compute_diff_size,
};
...
impl LocalDiffStateModel {
    ...
    async fn diff_metadata_against_specific_branch(
        ...
        // Also get untracked files, as they should be included in branch
        // comparisons. Bounded (see APP-5462): `--untracked-files=all` on a
        // repo with a huge non-gitignored directory tree (APP-4827) can
        // otherwise make this balloon to multiple GB before a single byte
        // is parsed.
        log::debug!(
            "[GIT OPERATION] local.rs diff_metadata_against_specific_branch git status --untracked-files=all --porcelain=2 -z"
        );
        let (status_files, files_truncated) = Self::bounded_git_status(
            repo_path,
            &["status", "--untracked-files=all", "--porcelain=2", "-z"],
        ...
        Ok(Self::build_diff_metadata_against_base(
            repo_path,
            changed_files,
            &num_stat_metadata,
            MAX_STATUS_ENTRIES,
    ...
    ///
    /// Bounded to `max_bytes` (see APP-5462): a repo with an enormous
    /// number of changed tracked files could otherwise make this balloon
    /// like `git status --untracked-files=all` does. On overshoot, only
    /// complete lines up to the cut are parsed — the per-file counts for
    /// tracked files past that point are simply missing from the returned
    /// map, which callers already treat as "no numstat entry" (falling
    /// back to 0 for tracked files, matching today's behavior for any file
    /// numstat doesn't report).
    ///
                ...
                .await
            {
                ...
                Ok(BoundedGitOutput::Exceeded(text)) => {
                    log::warn!(
                     ...
                     {max_bytes} bytes; some per-file line counts will be missing \
                     for this load"
                    );
    ...
    /// --untracked-files=all --porcelain=2 -z` normally would, but bounded
    /// to `METADATA_STATUS_BYTE_BUDGET`: a repo with an enormous
    /// non-gitignored directory tree (see APP-4827) can make that
    /// command's raw output multiple GB before a single byte is parsed
    /// (see APP-5462). Returns the parsed entries plus whether the read
    /// was cut short — in which case `entries` reflects only what fit in
    /// the budget, not the true set, so a caller must treat any count or
    /// total derived from it as a lower bound rather than exact.
    ///
    /// Unlike a byte-for-byte truncation of the raw text, this never hands
    /// a corrupted or partial record to [`Self::parse_git_status`]: on
    /// overshoot, any trailing record that was cut off mid-way is dropped
    /// rather than parsed, so every entry returned is a complete, valid
    /// record.
    ///
    /// `max_bytes` is a parameter (rather than hardcoding
    /// `METADATA_STATUS_BYTE_BUDGET`) purely so tests can force the
    /// overshoot path deterministically with a handful of files instead of
    /// needing to actually produce a multi-megabyte `git status`.
    async fn bounded_git_status(
        repo_path: &Path,
        args: &[&str],
        max_bytes: usize,
    ) -> Result<(Vec<(String, GitFileStatus)>, bool)> {
        match run_git_command_bounded(repo_path, args, max_bytes).await? {
            BoundedGitOutput::Complete(text) => Ok((Self::parse_git_status(&text)?, false)),
    ...
    /// Builds a `DiffMetadataAgainstBase` from `changed_files` (as parsed
    /// from `git status`/`git diff --name-status`) and `num_stat_metadata`
    /// (from `git diff --numstat`), retaining at most `max_retained`
    /// entries in the returned `files` list (see APP-5462: there is no
    /// point retaining metadata for more files than the diff-loading path
    /// would ever materialize a diff for in one load).
    ///
    /// `aggregate_stats.files_changed` and, for tracked files, the totals
    /// stay exact regardless of `max_retained` — summed from
    /// `num_stat_metadata` directly and from `changed_files.len()` before
    /// truncating, neither of which costs any extra work. Only *untracked*
    ...
    async fn build_diff_metadata_against_base(
        ...
        let mut total_additions: usize = num_stat_metadata.values().map(|m| m.lines_added).sum();
        let total_deletions: usize = num_stat_metadata.values().map(|m| m.lines_removed).sum();
        ...
        for (file_path, status) in &changed_files {
            let (additions, deletions) = if let Some(metadata) = num_stat_metadata.get(file_path) {
                ...
                // Degrade per entry: an uncountable untracked path (e.g. a
                // nested repo/worktree directory, or an unreadable file)
                // contributes 0 lines instead of failing the whole
                ...
                let lines = num_lines.unwrap_or(0);
...
pub(crate) async fn diff_metadata_against_head(
    ...
    // First, get the list of changed files with their status. Bounded (see
    // APP-5462): `--untracked-files=all` on a repo with a huge
    // non-gitignored directory tree (APP-4827) can otherwise make this
    // balloon to multiple GB before a single byte is parsed.
    let (changed_files, files_truncated) = LocalDiffStateModel::bounded_git_status(
        repo_path,
        &[
            "--no-optional-locks", // Avoid taking locks that might interfere with other git operations
            "status",
