...
use crate::code_review::diff_size_limits::{
    DiffSize, MAX_DIFF_SIZE, MAX_TOTAL_DIFF_BYTES, MAX_TOTAL_DIFF_FILES, UnrenderableReason,
    approx_file_diff_bytes, compute_diff_size,
};
...
impl LocalDiffStateModel {
    ...
    async fn diff_state_against_head(repo_path: &Path) -> Result<GitDiffWithBaseContent> {
        ...
        // A single `git diff --numstat` gives both cheap binary detection and
        // accurate per-file line counts — the latter needed by the aggregate
        // cap below to keep total_additions/total_deletions accurate even
        // for files it skips materializing.
        ...
        let (files, total_additions, total_deletions) = Self::materialize_with_aggregate_cap(
            ...
            |file_path, status, is_binary| async move {
                let mut file_diff =
                    Self::get_file_diff(repo_path, &file_path, &status, is_binary, None).await?;
                // Never read or ship base content for binary files: it can't
                // be inline-rendered and, after lossy UTF-8 decoding, can
                // balloon ~3x.
    ...
    /// Drives `fetch_file_diff` for each changed file, in order, but once the
    /// aggregate budget (`max_files` materialized entries, or `max_bytes`
    /// retained across their hunks and base content) is reached, stops
    /// calling it and instead pushes an "unrenderable" placeholder for every
    /// remaining file. This is what actually bounds a single load's memory
    /// and subprocess work when a repo has enough changed files that their
    /// aggregate is unbounded even though each individual file is small
    /// (see APP-5462) — `fetch_file_diff` does the expensive git subprocess
    /// calls and hunk parsing, and it is never invoked once the budget is
    /// exhausted.
    ///
    /// `fetch_file_diff` returns `Ok(None)` when a file turns out to have no
    /// real diff to show (e.g. a rename with no content change) and should
    ...
    async fn materialize_with_aggregate_cap<F, Fut>(
        ...
        max_files: usize,
        max_bytes: usize,
        mut fetch_file_diff: F,
    ) -> Result<(Vec<FileDiffAndContent>, usize, usize)>
    where
        F: FnMut(String, GitFileStatus, bool) -> Fut,
        Fut: std::future::Future<Output = Result<Option<FileDiffAndContent>>>,
    {
        let mut files = Vec::new();
        let mut total_additions = 0;
        let mut total_deletions = 0;
        let mut materialized_bytes = 0usize;
        let mut logged_cap = false;
        ...
        for (file_path, status) in changed_files {
            ...
            let is_binary = numstat
                .get(&file_path)
                .is_some_and(|metadata| metadata.is_binary_file);
            if let Some(file_diff) = fetch_file_diff(file_path, status, is_binary).await? {
                total_additions += file_diff.file_diff.additions();
                total_deletions += file_diff.file_diff.deletions();
                materialized_bytes += approx_file_diff_bytes(
    ...
    /// skipped fetching and parsing (see APP-5462). Mirrors the exact shape
    /// `get_file_diff` already returns for a single file whose own diff
    /// exceeds `MAX_DIFF_SIZE`, so the UI shows the same "Diff is too large
    /// to render" presentation instead of silently omitting the file.
    fn unrenderable_file_diff(file_path: &str, status: &GitFileStatus) -> FileDiffAndContent {
        FileDiffAndContent {
            file_diff: FileDiff {
    ...
    /// Logs once per load when the aggregate materialization budget
    /// (`MAX_TOTAL_DIFF_FILES`/`MAX_TOTAL_DIFF_BYTES`) is first exceeded, to
    /// avoid a log line per skipped file on repos with many changed files.
    fn log_aggregate_cap_once(logged: &mut bool, files_materialized: usize, bytes: usize) {
        if *logged {
            return;
        }
        *logged = true;
        log::warn!(
            "LocalDiffStateModel: reached aggregate diff materialization cap after {files_materialized} files ({bytes} bytes); remaining changed files will show as too large to render"
        );
    }
