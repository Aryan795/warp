...
fn trivial_file_diff(file_path: String, status: GitFileStatus) -> FileDiffAndContent {
    FileDiffAndContent {
        file_diff: FileDiff {
            file_path,
            status,
...
/// A single-line hunk of known, fixed size, so a test can pick a byte budget
/// that trips after an exact number of files.
fn single_line_file_diff(file_path: String, status: GitFileStatus) -> FileDiffAndContent {
    FileDiffAndContent {
        file_diff: FileDiff {
            file_path,
            status,
...
fn changed_files(count: usize) -> Vec<(String, GitFileStatus)> {
    (0..count)
        .map(|i| (format!("file{i}.txt"), GitFileStatus::Modified))
        .collect()
}

/// This is the property the aggregate cap exists to guarantee (APP-5462): once
/// the budget is reached, the expensive per-file work (`fetch_file_diff` —
/// standing in for the real git subprocess calls and hunk parsing) must not
/// run at all for the remaining files. A test that only checked the returned
/// shape would still pass even if every file were fetched first and only
/// truncated afterward, which is precisely the bug this guards against.
#[tokio::test]
async fn materialize_with_aggregate_cap_stops_fetching_once_file_cap_is_reached() {
    ...
    let (files, _, _) = LocalDiffStateModel::materialize_with_aggregate_cap(
        ...
        3,
        usize::MAX,
        move |file_path, status, _is_binary| {
            fetch_calls_clone.set(fetch_calls_clone.get() + 1);
            async move { Ok(Some(trivial_file_diff(file_path, status))) }
        },
    )
    .await
    ...
    assert_eq!(
        fetch_calls.get(),
        3,
        "fetch_file_diff must not be called once the file cap is reached"
    );
    assert_eq!(
        files.len(),
        10,
        "every changed file must still appear in the result"
    );
    let unrenderable_count = files
        .iter()
        .filter(|f| {
            matches!(
                f.file_diff.size,
        ...
        .count();
    assert_eq!(
        unrenderable_count, 7,
        "files beyond the cap must be marked unrenderable rather than silently dropped"
    );
}

/// Same property as the file-count cap test above, but for the byte budget:
/// `fetch_file_diff` must stop being called once the retained bytes reach
/// the budget, not just have its output discarded afterward.
#[tokio::test]
async fn materialize_with_aggregate_cap_stops_fetching_once_byte_cap_is_reached() {
    let numstat = HashMap::new();
    let fetch_calls = Rc::new(Cell::new(0));
    let fetch_calls_clone = fetch_calls.clone();
    let per_file_bytes = std::mem::size_of::<DiffLine>() + "x".len();
    ...
    let (files, _, _) = LocalDiffStateModel::materialize_with_aggregate_cap(
        ...
        move |file_path, status, _is_binary| {
            fetch_calls_clone.set(fetch_calls_clone.get() + 1);
            async move { Ok(Some(single_line_file_diff(file_path, status))) }
        },
    )
    .await
    .expect("materialize_with_aggregate_cap should not error");
    ...
    assert_eq!(
        fetch_calls.get(),
        3,
        "fetch_file_diff must not be called once the byte cap is reached"
    );
    assert_eq!(files.len(), 10);
}

/// Once the cap trips, `total_additions`/`total_deletions` must still
/// reflect every changed file (from the numstat already fetched), not just
/// the ones actually materialized — otherwise the summary silently
/// under-reports changes for files it stopped rendering.
#[tokio::test]
async fn materialize_with_aggregate_cap_uses_numstat_for_skipped_files_totals() {
...
#[test]
fn unrenderable_file_diff_matches_the_per_file_over_cap_presentation() {
    let placeholder =
        LocalDiffStateModel::unrenderable_file_diff("big.txt", &GitFileStatus::Modified);

    assert!(!placeholder.file_diff.is_binary);
    assert!(placeholder.file_diff.hunks.is_empty());
    assert_eq!(
        placeholder.file_diff.size,
        DiffSize::Unrenderable(UnrenderableReason::DiffTooLarge)
    );
    assert_eq!(placeholder.content_at_head, None);
}
