...
///
/// Without this, a mixed checkout would author every commit as whichever
/// identity happened to be first in the credential list. Local config rather
/// than a global `includeIf gitdir:` entry, so the identity stays attached to
...
/// 3. Merge refreshed credentials into `~/.git-credentials` and refresh the
///    matching provider CLI configs, leaving other hosts in place.
///
