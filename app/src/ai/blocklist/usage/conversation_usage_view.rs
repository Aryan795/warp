...
/// View to hold a conversation usage info block, rendered as the legacy
/// per-block "1B" usage pill in the agent footer.
pub struct ConversationUsageView {
    pub usage_info: ConversationUsageInfo,
    /// Optional timing information for the last set of responses.
...
impl ConversationUsageView {
    /// Test-only: builds a view without wiring up the rollup subscriptions
    /// that [`Self::new_footer_with_rollup`] needs a `ViewContext` for.
    #[cfg(test)]
    pub fn new(
    ...
    /// Constructs the view with orchestration credit rollup wired in. The
    /// view subscribes to
    ...
    /// when the parent conversation isn't known, or the orchestrator has no
    /// locally-loaded descendants with non-zero credits. The feature is
    /// self-gating: conversations without descendants short-circuit before
    /// any rollup-specific UI is built, so no feature flag is needed.
    ...
    /// Render the card container with the footer pill's chrome (rounded
    /// border, outer margin, and a top divider separating it from the
    /// block above).
