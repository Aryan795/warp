...
impl BlocklistAIController {
    ...
    pub(super) fn handle_response_stream_finished(
        ...
        // Per-request, non-persisted signal: the server pruned older attachments from this
        // turn's request to fit the resolved model's real limits. Not part of conversation
        // usage metadata (which persists and accumulates) since the same history may not need
        // pruning against a different model on a later turn — surface it once, quietly, and move
        // on rather than writing anything into the conversation itself.
            ...
            && let Some(window_id) = ctx.windows().active_window()
        {
            let message = if pruned_media.pruned_count == 1 {
                "1 older attachment is no longer visible to the model to stay within its request limits.".to_string()
            } else {
                format!(
                    "{} older attachments are no longer visible to the model to stay within its request limits.",
