...
impl Task {
    ...
    fn update_exchange_from_messages(
        ...
        let tool_calls = if should_convert_input_messages {
            Vec::new()
        } else {
            self.try_get_source()?
                .messages
                .iter()
                .chain(&messages)
                .filter_map(|message| message.tool_call().cloned())
                .collect()
        };
        ...
        if should_convert_input_messages {
            ...
            let mut output_action_ids = exchange
                .output_status
                .output()
                .into_iter()
                .flat_map(|output| {
                ...
                .collect::<HashSet<_>>();
            output_action_ids.extend(messages.iter().filter_map(|message| {
                message
                    .tool_call()
