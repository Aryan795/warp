...
fn mock_server_workflow_with_command(id: i64, name: &str, command: String) -> ServerWorkflow {
    ServerWorkflow::new(
...
/// Regression test for a panic where a workflow whose content preview has a multi-byte
/// character (emoji, CJK, accented char, etc.) straddling the fixed byte offset 197 would
/// cause `run_query` to panic with "byte index 197 is not a char boundary".
#[test]
fn run_query_does_not_panic_on_multibyte_content_preview() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);

        // 195 single-byte chars followed by a 4-byte emoji that straddles byte offset 197,
        // followed by more text so the preview exceeds the 200 byte truncation threshold.
        let mut content = "a".repeat(195);
        content.push('🚀');
        content.push_str(" more text after the emoji to push the preview past 200 bytes total");

        CloudModel::handle(&app).update(&mut app, |model, ctx| {
            model.upsert_from_server_workflow(
                mock_server_workflow_with_command(1, "multibyte workflow", content),
                ctx,
            );
        });

        let data_source = WorkflowDataSource::new();
        // Should not panic even though byte 197 falls in the middle of the emoji.
