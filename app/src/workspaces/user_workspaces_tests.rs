...
/// A window with no team must not switch a safety control off. The scope cannot distinguish a
/// user genuinely on no team from a window that was never given one, so both fall back to the
/// workspace's denylist: enforcing those rules is worse than enforcing the right team's, but far
/// better than enforcing nothing.
#[test]
fn test_sandboxed_denylist_falls_back_to_the_workspace_for_a_teamless_window() {
    let team = team_with_sandboxed_denylist(123, &["rm .*"]);
    let mut workspace = workspace_for_test(&team);
    workspace.settings.sandboxed_agent_settings = workspace_sandboxed_denylist(&["git .*"]);
    ...
    App::test((), |mut app| async move {
        ...
        app.read(|ctx| {
            assert_eq!(
                ...
                "a teamless window still gets the workspace's denylist, never an empty one"
            );
        });
    })
}
