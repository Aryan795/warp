...
/// Files at or above this size are rejected during Warp Drive import instead of being read
/// fully into memory (see APP-5426: an unbounded read of a large imported file was responsible
/// for multi-GB heap growth). Mirrors `MAX_ATTACHMENT_SIZE_BYTES` in `ai/attachment_utils.rs`;
/// a hand-authored notebook or workflow file is expected to be well under this bound.
const MAX_IMPORT_FILE_SIZE_BYTES: u64 = 10 * 1024 * 1024;

/// Reads `path` into memory for import, rejecting it once its size passes
/// [`MAX_IMPORT_FILE_SIZE_BYTES`]. The size is checked up front so an oversized file is never
/// opened, and the read itself is capped so a file that grows after that check still can't push
