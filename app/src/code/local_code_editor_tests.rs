...
fn auto_save_is_held_back_until_a_new_file_is_first_saved() {
    App::test((), |mut app| async move {
        ...
        // An existing file is eligible immediately.
        let existing = directory.path().join("already-there.md");
        std::fs::write(&existing, "# here").expect("write file");
        let editor = open_editor(&mut app, &existing).await;
        editor.read(&app, |editor, ctx| {
            assert!(!editor.is_new_file());
            assert!(editor.can_auto_save(ctx));
        });
    });
}
