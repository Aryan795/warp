    ...
    fn handle_action(&mut self, action: &CodeEditorViewAction, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            VimUserTyped(content) => {
                // Don't handle vim input when the find input has keyboard focus.
                let allow_vim = match &self.find_bar {
                    Some(find_bar) => {
                        let open = find_bar.as_ref(ctx).is_open();
                        let find_input_focused = find_bar.as_ref(ctx).is_find_input_focused(ctx);
                        !(open && find_input_focused)
                    }
                    None => true,
                };
                if allow_vim {
