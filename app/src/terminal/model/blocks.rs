...
impl BlockList {
    ...
    pub(super) fn apply_precmd_to_active(&mut self, data: PromptMetadata) {
        ...
        if self.skip_next_after_block_completed_event {
            ...
            // Hidden in-band blocks only need to survive through their completion event; retaining
            // their command/output grids causes unbounded memory growth in long sessions.
