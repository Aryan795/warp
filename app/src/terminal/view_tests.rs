...
use warpui::{App, EntityIdSet, Presenter, ReadModel, ViewHandle, WindowInvalidation};
...
fn initialize_app_for_attach_file_binding_tests(app: &mut App) {
    initialize_app_for_terminal_view(app);
    app.add_singleton_model(ImportedConfigModel::new);
    app.update(crate::terminal::init);
}

fn attach_file_binding_is_available(terminal: &ViewHandle<TerminalView>, app: &App) -> bool {
    terminal.read(app, |view, ctx| {
        ctx.editable_bindings_for_view(terminal.window_id(ctx), view.id())
            .iter()
            .any(|binding| binding.name == init::ATTACH_FILE_KEYBINDING)
    })
}

fn assert_attach_file_unavailable(terminal: &ViewHandle<TerminalView>, app: &mut App) {
    assert!(!attach_file_binding_is_available(terminal, app));
    assert!(!terminal.read(app, |view, ctx| view.can_attach_file(ctx)));
    terminal.update(app, |view, ctx| {
        view.handle_action(&TerminalAction::AttachFile, ctx);
    });
}

fn assert_attach_file_available(terminal: &ViewHandle<TerminalView>, app: &App) {
    assert!(attach_file_binding_is_available(terminal, app));
    assert!(terminal.read(app, |view, ctx| view.can_attach_file(ctx)));
}

fn cli_attach_file_session() -> CLIAgentSession {
    CLIAgentSession {
...
#[test]
fn attach_file_binding_is_absent_in_ordinary_terminal() {
    App::test((), |mut app| async move {
        initialize_app_for_attach_file_binding_tests(&mut app);
        FeatureFlag::AgentView.set_enabled(true);
        let terminal = add_window_with_terminal(&mut app, None);
        assert_attach_file_unavailable(&terminal, &mut app);
    });
}

#[test]
fn attach_file_binding_is_available_in_agent_view() {
    App::test((), |mut app| async move {
        initialize_app_for_attach_file_binding_tests(&mut app);
        FeatureFlag::AgentView.set_enabled(true);
        let terminal = add_window_with_terminal(&mut app, None);
        ...
        assert_attach_file_available(&terminal, &app);
    });
}

#[test]
fn attach_file_binding_is_available_in_inline_agent_view() {
    App::test((), |mut app| async move {
        initialize_app_for_attach_file_binding_tests(&mut app);
        FeatureFlag::AgentView.set_enabled(true);
        let terminal = add_window_with_terminal(&mut app, None);
        terminal.update(&mut app, |view, ctx| {
            ...
            view.agent_view_controller().update(ctx, |controller, ctx| {
                controller
                    ...
                    .expect("should enter inline agent view");
            });
        });
        assert_attach_file_available(&terminal, &app);
    });
...
#[test]
fn attach_file_binding_is_available_in_cli_agent_session() {
    App::test((), |mut app| async move {
        initialize_app_for_attach_file_binding_tests(&mut app);
        FeatureFlag::AgentView.set_enabled(true);
        let terminal = add_window_with_terminal(&mut app, None);
        terminal.update(&mut app, |view, ctx| {
            ...
            CLIAgentSessionsModel::handle(ctx).update(ctx, |sessions, ctx| {
                sessions.set_session(view_id, cli_attach_file_session(), ctx);
            });
        });
        assert_attach_file_available(&terminal, &app);
    });
...
#[test]
fn attach_file_binding_is_absent_for_shared_session_viewer() {
    App::test((), |mut app| async move {
        initialize_app_for_attach_file_binding_tests(&mut app);
        FeatureFlag::AgentView.set_enabled(true);
        let terminal = add_window_with_terminal(&mut app, None);
...
#[test]
fn attach_file_binding_is_available_for_cloud_session_viewer() {
    App::test((), |mut app| async move {
        initialize_app_for_attach_file_binding_tests(&mut app);
        FeatureFlag::AgentView.set_enabled(true);
        FeatureFlag::CloudMode.set_enabled(true);
        FeatureFlag::CloudModeImageContext.set_enabled(true);
