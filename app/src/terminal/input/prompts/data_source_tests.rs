...
/// prompt, then returns the ids the window is allowed to surface.
fn seed_prompts(app: &mut App, in_window_team: &Team, other_team: &Team) -> Vec<String> {
    ...
    [IN_WINDOW_TEAM_PROMPT_ID, PERSONAL_PROMPT_ID]
        .into_iter()
        .map(|id| SyncId::ServerId(WorkflowId::from(id).into()).uid())
        .sorted()
        .collect()
}
...
/// The zero-input `#` menu reads the cloud model directly rather than going through the Warp Drive
/// data source, so it needs its own window scoping.
#[test]
fn test_prompts_menu_zero_length_query_only_returns_prompts_in_the_window() {
    ...
    App::test((), |mut app| async move {
        ...
        let expected = seed_prompts(&mut app, &in_window_team, &other_team);
        ...
        assert_eq!(prompt_ids_for_query(&data_source, "", &app), expected);
    })
}

/// The single-character `#` menu takes a separate prefix-matching path that also reads the cloud
/// model directly.
#[test]
fn test_prompts_menu_single_character_query_only_returns_prompts_in_the_window() {
    ...
    App::test((), |mut app| async move {
        initialize_app(&mut app, vec![workspace]);
        // Every seeded prompt's name starts with "a", so only the window scoping can narrow this.
        let expected = seed_prompts(&mut app, &in_window_team, &other_team);
        ...
        assert_eq!(prompt_ids_for_query(&data_source, "a", &app), expected);
    })
}
...
fn test_prompts_menu_teamless_window_returns_personal_prompts() {
    ...
    App::test((), |mut app| async move {
        ...
        assert_eq!(
            ...
            vec![SyncId::ServerId(WorkflowId::from(PERSONAL_PROMPT_ID).into()).uid()]
        );
    })
}
