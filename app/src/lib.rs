...
pub fn run() -> Result<()> {
    ...
    // Server URL overrides are honored on internal dev channels and on Oss,
    // which points at a local adapter and must be repointable. Stable and
    // Preview silently ignore `--server-root-url` / `--ws-server-url` /
    // `--session-sharing-server-url` (and their `WARP_*` env-var equivalents)
    // so shipped builds can't be redirected away from their baked-in server
    // URLs. See `Channel::allows_server_url_overrides`.
