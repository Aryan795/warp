...
- The parent card renders only “Spawning N agents” while active and status-only copy after completion. It does not render child blockers or per-child errors in `app/src/ai/blocklist/inline_action/run_agents_card_view.rs:381-410,1202-1274,1557-1649`.
...
- Every consumer reads `Blocked` as “this action has not run and is waiting for the user to accept or reject it”. The renderers in `app/src/ai/blocklist/block/view_impl/output.rs:1352,2015,2745,2862,2948,3254` pair `is_blocked()` with `props.action_buttons.get(id).expect("Button states must exist for each requested action.")`, so an executing action reported as `Blocked` renders accept/reject buttons or panics. `AIBlock::is_blocked_on_user_confirmation` (`app/src/ai/blocklist/block.rs:4566-4571`) and `blocked_action_for_conversation` (`app/src/ai/blocklist/action_model.rs:541-566`) carry the same meaning.
