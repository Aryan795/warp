...
3. One eligible wheel notch creates a target scroll position at the same destination Warp uses
   today. Warp animates the displayed position to that target over 120 milliseconds.
   - The baseline easing is cubic ease-out.
   - The movement starts quickly and slows near the target.
   - The movement does not overshoot, bounce, or continue after it reaches the target.
   - Hands-on validation evaluates the duration. Any duration change must update this spec before
...
5. Repeated notches in the same direction compose into one continuous interaction.
   - Each notch adds its distance to the current target.
   - A new notch does not restart or pause movement that is already visible.
   - Earlier movement continues from its current progress.
...
14. Phase 2 applies the same 120-millisecond target animation to normal terminal block-list
    scrollback.
...
## Assumptions
- The approved 120-millisecond cubic ease-out is the initial tuning target.
