...
| `GET /api/v1/agent/connected-self-hosted-workers` | [`list_connected_self_hosted_workers`](https://github.com/warpdotdev/warp/blob/8b88df9874d8d632eba3bcfdd330acc9dabd23b0/app/src/server/server_api/ai.rs#L2187-L2194) sends no selector. | Accept `&TeamContext` and send the header on every request. |
...
|---|---|
| `FactoryClient::get_runners` | It returns all visible runners. Keep cross-team with owner metadata, or add a selected-team collection. |
| `GET /api/v1/agent?repo=...` skills | Decide whether skills are global, user-level, repository-level, or selected-team resources. |
...
Add request-local team propagation for `/ai/multi-agent`, `/agent/run`, `CreateAgentTask`, agent identity creation, connected workers, handoff snapshots, feature model choices, and artifact upload targets. This PR owns the GraphQL, public REST, and multi-agent transport changes needed for `X-Warp-Team-Uid`.
...
Use `TeamContext` for current-window managed-secret and API-key collection queries. Preserve explicit UIDs for team-owned secret mutations, encryption configuration, API-key creation, runner creation, and `oz secret --team`. This PR owns the personal-plus-selected-team response contracts, team-and-harness cache partitioning, CLI membership validation, and focused request tests. Personal-only mutations and task-scoped retrieval remain unchanged.
...
#### PR 2F: Resolve ambiguous collections
Decide the bucket 3 contracts and record them in this spec. Implement selected-team and cross-team changes in separate follow-up PRs so unresolved semantics do not block the mechanical bucket 1 and 2 migrations.
...
- **Cross-team collections mistaken for selected-team collections**: decide and document collection semantics before adding the capability.
## Follow-ups
- Add a typed server-client team-scope primitive if more crates need request-local team headers.
- Document the final selected-team versus cross-team semantics for the ambiguous collections in this spec before implementation reaches them.
