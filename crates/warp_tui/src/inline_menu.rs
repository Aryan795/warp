...
pub(crate) enum TuiInlineMenuRowStyle {
    Default,
    InlineMenuItem,
    /// A shell-command row: the title is prefixed with the green `!` shell
    /// affordance (without the transcript's shell-command background).
    ShellCommand,
}
...
pub(crate) enum TuiInlineMenuAccepted {
    ...
    Model(LLMId),
    Mcp(TuiMcpAction),
    /// A prompt or command accepted from the up-arrow history menu.
    History(UpArrowHistoryEntry),
...
impl TuiInlineMenuHandle for ModelHandle<TuiHistoryMenuModel> {
    fn mode(&self) -> TuiInputSuggestionsMode {
...
fn menu_result_row(
    ...
    let title = match row.style {
        ...
        // The green `!` shell affordance shared with the input prefix and the
        // transcript's command rows, minus the transcript's row background.
        TuiInlineMenuRowStyle::ShellCommand => {
            let mut prefix_style = builder.shell_command_accent_style();
            if is_selected {
                prefix_style = prefix_style.bg(builder.slash_command_selection_background());
            }
