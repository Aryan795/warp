...
use warp::tui_export::{
    AcceptSlashCommandOrSavedPrompt, AgentConversationEntryId, LLMId, TuiHistoryItem, TuiMcpAction,
};
...
pub(crate) enum TuiInlineMenuAccepted {
    ...
    Model(LLMId),
    Mcp(TuiMcpAction),
    /// A typed prompt or command accepted from the up-arrow history menu.
...
fn menu_result_row(
    ...
    let title_style = if is_selected {
        ...
        match (row.is_selectable, row.style) {
            ...
            (true, TuiInlineMenuRowStyle::Default | TuiInlineMenuRowStyle::HistoryCommand) => {
            ...
            (
                ...
                | TuiInlineMenuRowStyle::HistoryCommand,
            ) => builder.dim_text_style(),
        }
    };
    let show_description = match row.style {
        TuiInlineMenuRowStyle::Default | TuiInlineMenuRowStyle::HistoryCommand => {
    ...
    let title = match row.style {
        TuiInlineMenuRowStyle::Default | TuiInlineMenuRowStyle::HistoryCommand => single_line_title,
    ...
    let title = match row.style {
        TuiInlineMenuRowStyle::HistoryCommand => TuiText::from_spans([
            ("! ".to_owned(), builder.shell_command_accent_style()),
            (title, title_style),
        ])
    ...
    let mut content = TuiFlex::row()
        ...
        .child(match row.style {
            TuiInlineMenuRowStyle::Default | TuiInlineMenuRowStyle::HistoryCommand => title,
