...
use crate::model::iterm_image::{ITermImage, ITermImageMetadata};
...
pub trait Handler {
    ...
    /// Callback for the terminal when the shell reports the command selected in its raw-keypress
    /// ctrl-r handoff wrapper widget (see [`ExternalCtrlRRawKeypressSelectionValue`]).
    fn external_ctrl_r_raw_keypress_selection(
        &mut self,
        _data: ExternalCtrlRRawKeypressSelectionValue,
    ) {
    }
