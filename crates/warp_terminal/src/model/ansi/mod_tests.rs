...
impl Handler for MockHandler {
    ...
    fn external_ctrl_r_raw_keypress_selection(
        ...
        self.d_proto_hooks
            .push(DProtoHook::ExternalCtrlRRawKeypressSelection { value: data })
    }

    fn external_ctrl_r_raw_keypress_started(&mut self, data: ExternalCtrlRRawKeypressStartedValue) {
        self.d_proto_hooks
            .push(DProtoHook::ExternalCtrlRRawKeypressStarted { value: data })
    }
...
#[test]
fn parse_dcs_external_ctrl_r_raw_keypress_selection() {
    // Anti-regression for the ansi dispatch match in `Performer::handle_decoded_hook`: proves
    // the wire hook still reaches `Handler::external_ctrl_r_raw_keypress_selection` specifically
    // (not the sibling `..._started` arm, which takes a distinct value type).
    let bytes = hex_encoded_dcs_string(
        r#"{
                "hook": "ExternalCtrlRRawKeypressSelection",
                "value": {
    ...
    assert_eq!(handler.d_proto_hooks.len(), 1);
    match handler.d_proto_hooks.first().unwrap() {
        DProtoHook::ExternalCtrlRRawKeypressSelection { value } => assert_eq!(
            *value,
            ExternalCtrlRRawKeypressSelectionValue {
                buffer: "echo selected".to_string(),
                token: "1001".to_string(),
                session_id: Some(167303092612201),
            }
        ),
        _ => panic!("incorrect dcs value"),
...
#[test]
fn parse_dcs_external_ctrl_r_raw_keypress_started() {
    // Anti-regression counterpart to `parse_dcs_external_ctrl_r_raw_keypress_selection`: proves
    // the wire hook reaches `Handler::external_ctrl_r_raw_keypress_started`, not the selection
    // arm.
    let bytes = hex_encoded_dcs_string(
        r#"{
                "hook": "ExternalCtrlRRawKeypressStarted",
                "value": {
                    "token": "1001",
                    "session_id": 167303092612201
                }
    ...
    assert_eq!(handler.d_proto_hooks.len(), 1);
    match handler.d_proto_hooks.first().unwrap() {
        DProtoHook::ExternalCtrlRRawKeypressStarted { value } => assert_eq!(
            *value,
