...
mod tests {
    ...
    fn debug_redacts_sensitive_fields() {
        let failure = DiffApplicationFailure::UnmatchedDiffs {
            file: "secret/path.rs".to_string(),
            fuzzy_match_failure_count: 1,
            changes_already_applied_count: 0,
            search_block_failures: vec![DiffSearchBlockFailure {
                search: "super secret search text".to_string(),
                expected_range: Some(1..5),
            }],
        };
        let debug_str = format!("{failure:?}");
        assert!(
            !debug_str.contains("secret/path.rs"),
            "file path must not appear in Debug output"
        );
        assert!(
            !debug_str.contains("super secret search text"),
            "search text must not appear in Debug output"
        );
        assert!(debug_str.contains("<redacted>"));

        let block = &failure.search_block_failures().unwrap()[0];
        let block_debug = format!("{block:?}");
        assert!(
            !block_debug.contains("super secret search text"),
            "search block must not appear in Debug output"
        );
    }

    // Helper used only in tests.
    impl DiffApplicationFailure {
        fn search_block_failures(&self) -> Option<&Vec<DiffSearchBlockFailure>> {
            match self {
                DiffApplicationFailure::UnmatchedDiffs {
                    search_block_failures,
                    ..
                } => Some(search_block_failures),
                _ => None,
            }
        }
    }
